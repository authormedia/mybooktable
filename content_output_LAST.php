...
global $post, $options;
	...
	$paypal_variations_label = get_post_meta($post->ID, '_cmb_paypal_variations_label', true);
	
	$paypal_variation_name_1 = get_post_meta($post->ID, '_cmb_paypal_variation_name_1', true);
	$paypal_variation_price_1 = get_post_meta($post->ID, '_cmb_paypal_variation_price_1', true);

	$paypal_variation_name_2 = get_post_meta($post->ID, '_cmb_paypal_variation_name_2', true);
	$paypal_variation_price_2 = get_post_meta($post->ID, '_cmb_paypal_variation_price_2', true);

	$paypal_variation_name_3 = get_post_meta($post->ID, '_cmb_paypal_variation_name_3', true);
	$paypal_variation_price_3 = get_post_meta($post->ID, '_cmb_paypal_variation_price_3', true);

	$paypal_variation_name_4 = get_post_meta($post->ID, '_cmb_paypal_variation_name_4', true);
	$paypal_variation_price_4 = get_post_meta($post->ID, '_cmb_paypal_variation_price_4', true);

	$paypal_variation_name_5 = get_post_meta($post->ID, '_cmb_paypal_variation_name_5', true);
	$paypal_variation_price_5 = get_post_meta($post->ID, '_cmb_paypal_variation_price_5', true);
	// End Paypal Vars
		
if($main_options['use_amazon']=='on'){
	$buttoncode_ama = get_post_meta($post->ID, '_cmb_button_code_amazon', true);
}
if($main_options['use_cbd']=='on'){
	$buttoncode_cbd = get_post_meta($post->ID, '_cmb_button_code_cbd', true);
}
if($main_options['use_bnn']=='on'){
	$buttoncode_bnn = get_post_meta($post->ID, '_cmb_button_code_bnn', true);
}
if($main_options['use_ejunkie']=='on'){
	$buttoncode_ejunk = get_post_meta($post->ID, '_cmb_button_code_ejunkie', true);
}
if($main_options['use_kickstart']=='on'){
	$buttoncode_kick = get_post_meta($post->ID, '_cmb_button_code_kickstart', true);
}
if($main_options['use_bnn_nook']=='on'){	
	$buttoncode_nook = get_post_meta($post->ID, '_cmb_button_code_nook', true);
}

if($main_options['use_kindle']=='on'){	
	$buttoncode_kindle = get_post_meta($post->ID, '_cmb_button_code_kindle', true);
}
if($main_options['use_bim']=='on'){	
	$buttoncode_bim = get_post_meta($post->ID, '_cmb_button_code_bim', true);
}
if($main_options['use_sba']=='on'){	
	$buttoncode_sba = get_post_meta($post->ID, '_cmb_button_code_sba', true);
}


if($main_options['use_paypal']=='on'){	
	$buttoncode_paypal = get_post_meta($post->ID, '_cmb_button_code_paypal', true);
}	
	//$prodcats
	...
	if (!empty($buttoncode_ama)){
		...
		$is_button = find_img_tag($buttoncode_ama); 
			if ($is_button){
				...
				$proddetails .= '<div class="prodbutton">'.$ama_button .'</div>';
			} else { 
	...
	if (!empty($buttoncode_cbd)){
		...
		$is_button = find_img_tag($buttoncode_cbd); 
			if ($is_button){
				...
				$proddetails .= '<div class="prodbutton">'.$cbd_button .'</div>';
			} else { 
				// if not then add the default image			
	...
	if (!empty($buttoncode_bnn)){
		...
		$is_button = find_img_tag($buttoncode_bnn); 
			if ($is_button){
				...
				$proddetails .= '<div class="prodbutton">'.$bnn_button .'</div>';
			} else { 
				// if not then add the default image			
	...
	if (!empty($buttoncode_ejunk)){
		...
		$is_button = find_img_tag($buttoncode_ejunk); 
			if ($is_button){
				...
				$proddetails .= '<div class="prodbutton">'.$ejunk_button .'</div>';
			} else { 
				// if not then add the default image			
	...
	if (!empty($buttoncode_kick)){
		...
		$is_button = find_img_tag($buttoncode_kick); 
			if ($is_button){
				...
				$proddetails .= '<div class="prodbutton">'.$kick_button .'</div>';
			} else { 
				// if not then add the default image			
	...
	if (!empty($buttoncode_nook)){
		...
		$is_button = find_img_tag($buttoncode_nook); 
			if ($is_button){
				...
				$proddetails .= '<div class="prodbutton">'.$nook_button .'</div>';
			} else { 
				// if not then add the default image			
	...
	if (!empty($buttoncode_kindle)){
		...
		$is_button = find_img_tag($buttoncode_kindle); 
			if ($is_button){
				...
				$proddetails .= '<div class="prodbutton">'.$kindle_button .'</div>';
			} else { 
				// if not then add the default image			
	...
	if (!empty($buttoncode_bim)){
		...
		$is_button = find_img_tag($buttoncode_bim); 
			if ($is_button){
				...
				$proddetails .= '<div class="prodbutton">'.$bim_button .'</div>';
			} else { 
				// if not then add the default image			
	...
	if (!empty($buttoncode_sba)){
		...
		$is_button = find_img_tag($buttoncode_sba); 
			if ($is_button){
				...
				$proddetails .= '<div class="prodbutton">'.$sba_button .'</div>';
			} else { 
				// if not then add the default image			
...
$proddetails .= '<div class="prodbutton">';
...
//require_once(WP_PLUGIN_DIR . "/" . basename(dirname(__FILE__)) . "/excerpt_output.php");
		...
		$output .= '<div class="name">'.$name.'</div>';
			...
			$relatedbooks = new WP_Query( array( 'product_collection' => $slug, 'post__not_in' => array($postID) ) );
				while ($relatedbooks->have_posts()) : $relatedbooks->the_post();
					...
					$buttoncode = buttoncode(get_the_id());
					$output .= '<div class="book"><a href="'.get_permalink().'">'.get_the_title().'</a></div>';
					$output .= '<div class="book">'.$buttoncode.'</a></div>';
