...
function mbt_default_affiliate_editor($data, $id, $affiliates) {
	...
	echo('<b>'.$affiliates[$data['type']]['name'].':</b><br><textarea name="'.$id.'[value]" cols="60" rows="10">'.$data['value'].'</textarea><p>'.$affiliates[$data['type']]['desc'].'</p>');
}
...
function mbt_default_affiliate_button($data) {
	$value = $data['value'];
	$type = $data['type'];
	return empty($value) ? '' : '<div class="mbt-product-button">'.(mbt_find_img_tag($value) ? mbt_change_button_image($value, $type) : mbt_add_button_image($value, $type)).' </div>'; 
}

function mbt_add_basic_affiliates($affiliates) {
	$affiliates['amazon'] = array('name' => 'Amazon', 'desc' => '(optional) paste in the button code for this item', 'editor' => 'mbt_default_affiliate_editor', 'button' => 'mbt_default_affiliate_button');
