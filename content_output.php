...
global $post, $options;
	...
	if (!empty($main_options['readmorebutton'])){
		$readmorebutton = $main_options['readmorebutton'];		
		} else {
			$readmorebutton = 'More Details &raquo;';
		}
	// excerpt button styles
	// if this is woo, we can add button styles -- 	if ($main_options['is_woo']=='on') {
...
*/	
	...
	$theprice = price_output($post->ID,$main_options['use_kickstart'],$main_options['use_paypal']); 
//	print_r($theprice);
...
*/
	// price should output according to the rules set in price_output(), currently only show if cart is kickstart or paypal. blocking output if paypal is using variations should also go into that function but for now we check that here
	...
	//if (!empty($buttoncode_paypal) && !empty($theprice)  && !empty($paypalemail) && $paypal_variations_toggle != 'on'){
	//if ((!empty($price) || $price > 0) && $main_options['use_paypal']=='on' && $paypal_variations_toggle != 'on'){
	//	$proddetails .= '<div class="price"><h3>Paypal Price: '.$theprice.'</h3></div>';
//	}	
...
if ($main_options['readmore_buttons_placement']=='above'){ 
	if (is_archive()) {	
...
if ( !is_archive() || (is_archive() && $main_options['buttons_in_archive']=='on') ) {
$proddetails .=  buttoncode($post->ID,$theprice); // this is where all buttons are added
//$proddetails .= $post->ID;
}
...
if ( (is_archive() && $main_options['related_in_excerpts']=='on') || (!is_archive() && $main_options['related_in_content']=='on')) {
$proddetails .= relatedproducts($post->ID);
}
// End Related Products

// archives get the read more link below the buttons
if ($main_options['readmore_buttons_placement']=='below'){ 
	if (is_archive()) {	
...
if ($main_options['use_issuu']=='on'){ // This needs to be created in main options
...
$proddetails .= '
<div>
<object style="width:640px;height:472px" >
<param name="movie" value="http://static.issuu.com/webembed/viewers/style1/v2/IssuuReader.swf?mode=mini&amp;embedBackground=%23000000&amp;backgroundColor=%23222222&amp;documentId='.$docID.'" />
<param name="allowfullscreen" value="true"/>
<param name="menu" value="false"/>
<param name="wmode" value="transparent"/>
<embed src="http://static.issuu.com/webembed/viewers/style1/v2/IssuuReader.swf" type="application/x-shockwave-flash" allowfullscreen="true" menu="false" wmode="transparent" style="width:640px;height:472px" flashvars="mode=mini&amp;embedBackground=%23000000&amp;backgroundColor=%23222222&amp;documentId='.$docID.'" />
</object>
</div>
';
...
add_action('wp_head','load_buttoncode');

function buttoncode($postID,$theprice='') {
	global $post, $options;
	...
	if (!empty($buttoncode_paypal) && !empty($theprice)  && !empty($paypalemail)){
